// shared.js — Room For More v2 (white + brand fonts + real logos)
(function () {
...
<nav class="nav" role="navigation" aria-label="Main">
  <div class="nav__inner">
    <a href="index.html" class="nav__logo" aria-label="Room For More home">
      <img src="rfm_main_logo.PNG" alt="Room For More logo">
    </a>
  ...
  const tickerWords = ['MORE ACCESS','MORE EXPOSURE','MORE OPPORTUNITY','MORE CONFIDENCE','MORE POSSIBILITY','MORE REPRESENTATION','MORE COMMUNITY','MORE FUTURES'];
  const tickerItems = Array(2).fill(tickerWords.map(t=>`<span class="ticker__item">${t}<span class="ticker__dot"></span></span>`).join('')).join('');
  const TICKER_HTML = `
<div class="ticker" aria-hidden="true">
  <div class="ticker__track">${tickerItems}</div>
</div>`;
...
<footer class="footer" role="contentinfo">
    ...
    <div class="footer__top">
      <div>
        <div class="footer__logo">
          <img src="rfm_alt_logo.png" alt="Room For More">
        </div>
  ...
  document.getElementById('nav-placeholder')?.outerHTML && (document.getElementById('nav-placeholder').outerHTML = NAV_HTML);
  document.getElementById('ticker-placeholder')?.outerHTML && (document.getElementById('ticker-placeholder').outerHTML = TICKER_HTML);
  document.getElementById('footer-placeholder')?.outerHTML && (document.getElementById('footer-placeholder').outerHTML = FOOTER_HTML);
  ...
  document.addEventListener('click', function(e) {
    ...
    if (amountInput && card.dataset.amount) amountInput.value = card.dataset.amount;
  });
  ...
  document.addEventListener('click', function(e) {
    ...
    if (!header) return;
    header.closest('.program-full').classList.toggle('open');
  });

  // Email signup feedback
  document.addEventListener('submit', function(e) {
    if (!e.target.classList.contains('email-form')) return;
    e.preventDefault();
  ...
  // Card number format
  const cardInput = document.getElementById('d-card');
  ...
  // Expiry format
  const expInput = document.getElementById('d-exp');
